    ...
    def request(self, method, url, max_retry: int = 2,
        ...
        for i in range(max_retry + 1):
            try:
                ...
                requestCookieCensor = re.sub(r'(_MHYUUID|login_ticket|account_id|cookie_token|ltoken|ltuid)([ ]*=[ ]*)([a-zA-Z0-9+_\-]+)([ ]*;)',r'\g<1>\g<2>********\g<4>',requestHeaders.get('Cookie'),flags=re.IGNORECASE)
                requestHeaders.update({'Cookie':requestCookieCensor})
                print("Request Headers: ", requestHeaders)

                #Response Headers
                responseHeaders = response.headers
                responseCookieCensor = re.sub(r'(_MHYUUID|login_ticket|account_id|cookie_token|ltoken|ltuid)([ ]*=[ ]*)([a-zA-Z0-9+_\-]+)([ ]*;)',r'\g<1>\g<2>********\g<4>',responseHeaders.get('Set-Cookie'),flags=re.IGNORECASE)
                responseHeaders.update({'Set-Cookie':responseCookieCensor})
